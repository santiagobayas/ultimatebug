...
saludar("Alex")  # Llama a la función</pre>
    ...
    const LEVELS = [
        ...
        },
        {
            ...
            platforms: [
                ...
                { x: 360, y: 480, w: 600, h: 60, type: 'ground' },
                { x: 100, y: 370, w: 120, h: 20 },
                { x: 250, y: 290, w: 130, h: 20 },
                { x: 440, y: 360, w: 140, h: 20 },
                { x: 640, y: 270, w: 150, h: 20 }
            ],
            coins: [
                { x: 120, y: 320, collected: false },
                { x: 340, y: 240, collected: false },
                { x: 460, y: 310, collected: false },
                { x: 720, y: 220, collected: false },
                { x: 880, y: 430, collected: false }
            ],
            interactables: [
                { x: 260, y: 250, w: 32, h: 40, type: 'terminal', challengeId: 'terminal_4_1', resolved: false, label: 'Corchetes' },
                { x: 520, y: 320, w: 32, h: 40, type: 'terminal', challengeId: 'terminal_4_2', resolved: false, label: 'Append' },
                { x: 770, y: 400, w: 24, h: 80, type: 'door', challengeId: 'door_4', resolved: false, label: 'Puerta [0]' }
            ],
            enemies: [
                { x: 460, y: 334, minX: 440, maxX: 560, vx: 1.8, w: 26, h: 26, alive: true },
                { x: 660, y: 244, minX: 640, maxX: 770, vx: 2.0, w: 26, h: 26, alive: true },
                { x: 480, y: 454, minX: 380, maxX: 680, vx: 2.2, w: 26, h: 26, alive: true }
            ],
            goal: { x: 910, y: 420, w: 36, h: 60 }
        },
        {
    ...
    function loadLevel(index) {
        ...
        if (level.isBossLevel) {
            ...
            window.soundSystem.startBossBgm(); // MÚSICA DE JEFE
        } else {
            bossHud.classList.add('hidden');
            window.soundSystem.startStageBgm(); // MÚSICA DE ETAPA NORMAL
        }
    }
    ...
    function renderShop() {
        ...
        SKINS_CATALOG.forEach(skin => {
            ...
            if (isEquipped) {
                ...
                buttonHtml = `<button class="shop-action-btn equip" data-skin="${skin.id}">EQUIPAR</button>`;
            } else {
                const canAfford = coins >= skin.price;
                buttonHtml = `<button class="shop-action-btn buy" data-skin="${skin.id}" data-price="${skin.price}">${canAfford ? `COMPRAR (${skin.price} 🪙)` : `🔒 ${skin.price} 🪙`}</button>`;
            }

            card.innerHTML = `
                <div class="shop-avatar-preview">${skin.icon}</div>
            ...
            if (buyBtn) {
                buyBtn.addEventListener('click', () => {
                    if (coins >= skin.price) {
                        ...
                        window.soundSystem.playError();
                        alert(`¡Te faltan ${skin.price - coins} monedas para comprar esta skin! Recoge más en los niveles.`);
                    }
                });
            }
